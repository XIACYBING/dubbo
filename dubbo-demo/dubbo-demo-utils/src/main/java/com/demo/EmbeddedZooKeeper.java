...
public class EmbeddedZooKeeper {
    ...
    private static int findRandomPort(int min, int max) {
        ...
        List<Integer> randomInts =
            RANDOM.ints(min, max + 1).limit(max - min).mapToObj(Integer::valueOf).collect(Collectors.toList());
     ...
     *
     * @param port   监听端口
     * @param daemon 启动zk的线程是否守护线程
     * @return 返回启动完成的zk实例
     */
    public static EmbeddedZooKeeper start(int port, boolean daemon) {

        // 初始化zk
        EmbeddedZooKeeper zk = new EmbeddedZooKeeper(port, daemon);
